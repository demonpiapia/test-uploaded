...
6. 不自行增删监测项。解析时还必须校验第 6 列为小写 `yes|no`、`#` 为整数、repo 不重复、`## 监测列表` 唯一存在且表头与固定 6 列 contract 一致。
...
- 每个数据行必须 exactly 6 列。
- 第 1 列必须为整数序号；第 2 列必须是 `[项目名](https://github.com/{owner}/{repo}/releases)` 形式的 releases 链接；第 6 列只允许小写 `yes|no`。
- `owner/repo` 不得重复。
...
if ($monitorSections.Count -eq 1) {
    ...
    foreach ($row in $dataRows) {
        ...
        if ($c[5] -notmatch '^(yes|no)$') { $parseErrors += ('第 6 列 flag 非法（仅允许 yes/no）：{0}' -f $t); continue }
...
$stats=[PSCustomObject]@{
    ...
    yes=@($final|Where-Object flag -eq 'yes').Count
...
$badFlag=@($rows2|Where-Object{$_-notmatch '\|\s*(yes|no)\s*\|\s*$'})
...
$headerOk=($check -split "`r?`n"|Where-Object{$_.Trim() -eq $header}).Count -eq 1
